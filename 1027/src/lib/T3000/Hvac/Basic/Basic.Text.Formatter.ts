...
class Formatter {
  ...
  RenderUnderline(e, t) {
    ...
    a.plot(
      'M' + Utils1.RoundCoord(e.x) + ',' + (Utils1.RoundCoord(e.y) + 2) + 'h' + Utils1.RoundCoord(e.width)
    ),
  ...
  RenderBullet(e, t) {
    ...
    switch (e.bullet) {
      case 'hround':
        (
          S = this.parent.doc.CreateShape(ConstantData.CreateShapeType.OVAL)
        ).SetSize(n, n),
          l = !0;
        break;
      case 'sround':
      ...
      case 'hsquare':
        (
          S = this.parent.doc.CreateShape(ConstantData.CreateShapeType.RECT)
        ).SetSize(n, n),
          l = !0;
        break;
      case 'ssquare':
      ...
      case 'diamond':
        (
          S = this.parent.doc.CreateShape(ConstantData.CreateShapeType.POLYGON)
        ).SetPoints([{
          x: n / 2,
          y: 0
        },
      ...
      case 'check':
        (
          S = this.parent.doc.CreateShape(ConstantData.CreateShapeType.POLYLINE)
        ).SetPoints([{
          x: 0,
          y: n - n / 4
        },
      ...
      case 'plus':
        (
          a = (
            S = this.parent.doc.CreateShape(ConstantData.CreateShapeType.PATH)
          ).PathCreator()
        ).MoveTo(0, n / 2),
          a.LineTo(n, n / 2),
          a.MoveTo(n / 2, 0),
